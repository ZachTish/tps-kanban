...
Canonical source, tests, Git metadata, and dependency links live in this test vault under `Plugin Development`. Stable work uses `TPS-Kanban (Dev)` on `main`; optimization work uses the separate `TPS-Kanban (Optimize)` worktree on `optimization`. Stable builds may deploy byte-changed runtime artifacts only to this test vault. Optimization builds are build-only and report `[runtime-deploy] target=none lane=optimization`, so they cannot replace the installed test or production plugin. Promotion to `/Users/zachtisherman/TishOS v0.1/.obsidian/plugins/tps-kanban` remains an explicit guarded post-validation action, and deployment never overwrites `data.json` or other runtime-owned state.
...
- Confirmed task and bullet drops are compare-and-apply mutations. Kanban reads an uncached source snapshot before showing the confirmation preview, then commits inside `vault.process` only if the entire source note is still identical. If the note changed while confirmation was open, Kanban leaves it untouched, reports the stale plan, and asks the user to retry. This deliberately fails closed rather than overwriting a newer edit or a different line after an insertion.
...
- 2026-07-19 optimization: Made confirmed task and bullet drag/drop plans fail closed when their source note changes while the confirmation modal is open. The plan now comes from an uncached snapshot, the atomic apply verifies the full snapshot and original line, and stale plans leave the file byte-identical with a retry notice instead of applying an old result. Behavioral coverage includes normal apply, an edited task, an insertion above identical duplicate tasks, and CRLF preservation. Validation: focused utility suite, all 52 declared tests, TypeScript check, and the required separate optimization build. Both builds reported `[runtime-deploy] target=none lane=optimization`; no test or production runtime was deployed or reloaded.
